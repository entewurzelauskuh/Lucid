...
> **The images here are concept art, not screenshots.** Nothing was captured from a running build: they are AI-generated, made to pin down the look and the feel while the game is written. Some of what they show exists today, most of it is milestones away, and each caption says which. `docs/concept/README.md` has the details.
...
**The bedroom.** The start cube: fixed, exempt from every rule, and where Sleepers spawn and respawn. Its door is sealed by mist until the head start ends. *Built.*
...
**Grey beside you, white ahead.** Every doorway with nothing behind it is a **fog door**. Grey is closed and solid to the touch; white is the way out. Neither reads by hue alone. The white doors are the deepest in the dream, and they move further away each time the Nightmare builds on them. A room you have set foot in hardens its remaining doors into wall for good — in *every* Sleeper's dream at once, so the maze can only grow where nobody has been yet. *Built — `docs/SPEC.md` §7, and the mist is a generated-noise shader.*
...
**The Sleeper** gets a torch, a timer and the two numbers that matter: how deep they are, and how deep the exit is. Every rule change that affects them says so out loud. *The HUD is M0; the round flow that fills it in is M0.9.*
...
**Scale is the dream's to choose.** *M4, in a Dream Pack.*
...
![A brass revolver firing at the glowing weak point beneath a large mechanical eye mounted on a stone wall](docs/concept/chicane-weak-point.jpg)

**Every trap has a weak point.** Shoot the tell and the room stops trying to kill you. *M2, on the chicane framework in `docs/CHICANES.md`.*
