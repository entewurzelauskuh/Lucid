...
Context: #79, M0.6b. The first screens, and the structure every later one hangs off. Three choices deviate from what the design system's guide or the plan literally says, and one settles a default.
Decision: (1) **Fonts are referenced as TTFs, not as a FontAsset per weight.** The guide's §1 lists `*.asset FontAsset per weight`; the tokens file already points `-unity-font-definition` at the `.ttf` files, and UI Toolkit builds the atlas at run time from those. So there is nothing to generate and nothing to drift. FontAssets come back the day atlas control matters — a fixed glyph set, SDF padding for a very large wordmark — and get their own generator then. (2) **The Sandbox state is a shell with a Sleeper standing in the bedroom**, and Esc goes back. The plan puts the god view in M0.7 and the F5 switch in M0.9b; this is the state they fill, built so that going there and back is already a round trip the acceptance can measure. (3) **`Loading` is a flag, not a fourth state.** Nothing is ever *in* loading; it is between two things, and a state the table would have to allow into and out of from everywhere is a state a screen can get stuck in. `GameFlow.IsTransitioning` is true while scenes swap and a second `Go` throws. (4) **The input asset gained a `UI` map with one action, `Back`, on Esc.** A map per state was the plan; two states need two maps, and the Sleeper's already existed.
Reason: each is the smallest thing that meets M0.6b's acceptance without deciding something M0.7 or M1 owns. The Title's backdrop is the real bedroom through `DreamInstance` rather than an image, because docs/UI.md §3 says "the start cube, a bedroom at night" and the cube exists; its door shows as Exit, the bare lattice's deepest fog door, and §3's grey-pulsing-white is decorative polish deferred with the rest of pulse four. The `PanelSettings` and the three scenes are written by editor scripts and compared by signature, so `verify-generated.sh` covers them, and `EditorBuildSettings` is written the same way because loading by name needs it and a list nobody regenerates is a list somebody edits by hand.
Spec: none. `CLAUDE.md` Status and repository map.

## 2026-09-07 — The UI design system is adopted, with its defaults written down

Context: #85. Claude Design delivered a design system for the interface (`design-system/`): tokens, components, an icon language, every screen at 1920×1080, and a Unity-facing guide. Its `design-system/guidelines/decisions-draft.md` lists nineteen defaults it chose where `docs/UI.md` was open, and its change request asked for `docs/` to say the same things so the implementation could be consistent with itself.
Decision: the design system is the visual source of truth and `design-system/unity/CLAUDE-CODE-UI-GUIDE.md` the implementation authority for how the interface looks; `docs/UI.md` wins where they disagree. Its files live under `Assets/_Lucid/Runtime/UI/` inside `Lucid.Runtime`, namespace `Lucid.Runtime.UI`, not the guide's original `Assets/_Lucid/UI/`: every script in this project is in an assembly definition, and a folder outside them lands in `Assembly-CSharp`, which `Lucid.Runtime` cannot reference. The nineteen defaults stand, and each is written into `docs/UI.md` where its subject lives — the HUD scrim in §6, the docked panels in §8, the rest in §15 — rather than left in a draft file: translucency allocated by lifetime; modal scrims split into frozen and live; five type sizes with the 22 px floor for every *value* on Results and the scoreboard; tabular and lining figures from two extra font files; the three Sleeper-colour rules (raw chip fill, never used as text, the Nightmare reserves the chip footprint); the connector net generated from the mask; dashed borders as one 9-sliced sprite, the net using none; no logo, LUCID set in the display face; blocked buttons state their blocker; HUD clusters lit by a radial scrim rather than framed in panels; both Nightmare docks full height; colour-blind marker shapes an option, off by default; scoreboard columns min-width; the rank digit at `--fg-3`; Results derived from one session record; `--fg-4` decorative or ≥ 24 px only. Dark dims at two levels — non-rule chrome to 32 %, the health ring and crescent lives held at 60 % — because a flat 32 % measured about 1.7:1 on the two readouts a rule depends on; the numbers were the design system's and the owner confirmed them, together with the general clause behind them, which is now `docs/UI.md` §1's sixth principle: anything a rule depends on reading stays readable, above 4.5:1 and unanimated while it is read. Dark's bite lives in the world, where the lights go out, so holding the HUD costs the power nothing.
...
Spec: `docs/UI.md` §1 (principle 6), §6 (Dark's two levels), §8, §9, §11, §14, §15; `docs/SPEC.md` §15, §18, §21; `docs/UI-TOOLKIT-LIMITS.md` new. One existing §14 string changed, by its own entry below; the rest gained the strings other sections already carried and six section captions.

## 2026-09-07 — The Nightmare possesses *the* mob, not *a* mob

Context: #85. `docs/UI.md` §14's possession line read "Possessing a {mob} in {name}'s dream — P to let go". Spec §8 names two possessable mobs, the Shade and the Eye, and with the second the line reads "Possessing a Eye".
Decision: the article is *the*. The string, §8's overlay example and `LucidStrings.Possessing` change together; nothing else about the line does.
Reason: four ways out were weighed. The pack supplying the noun with its article moves grammar into content files, where every pack author has to get it right; an article helper in code is a heuristic for a two-entry problem; renaming the Eye contradicts CHICANES, the icons and the mockups. *The* is not only safe but more accurate — the Nightmare possesses a specific mob under the cursor, not one of a kind. Recorded because §14's strings are the rules made visible and a change to one is never made in passing.
Spec: `docs/UI.md` §8, §14.
...
Decision: concept art is project work under CC-BY-4.0, named in §18 alongside the specs. It lives in `docs/concept/` at web resolution, and `docs/concept/README.md` records for each image how it was made — these were generated with Google Gemini. The full-resolution masters are not committed: they are 24 MB the set, they would sit in history for ever, and the README needs 1.8 MB of web-sized copies instead. `docs/concept/*.jpg` is exempted from Git LFS in `.gitattributes`, because GitHub serves README images through the LFS bandwidth quota and ten of them on the front page would spend it on people merely reading about the game. That exemption is conditional on these files being **write-once**: an LFS clone fetches only the version it checks out, a plain blob is fetched for all of history for ever, so one re-export of the set would take a fresh clone from 2.4 MB to 4.2 MB and every later one would add as much again. Replacing this art rather than adding to it means moving these back into LFS, or taking that cost knowingly.
Reason: the licence has to say something, and the honest thing for it to say is that this is the project's own material rather than someone else's. Recording the provenance is the other half: a contributor who wants to know whether an image is a hand-drawn asset, a screenshot or a generated mock-up should be able to find out without asking, and the README's own note that these are **not screenshots** is load-bearing — the god view and the mobs in them do not exist, and a reader who took them for captures would badly misjudge how far along the project is.
Spec: `docs/SPEC.md` §18's licensing bullet names concept art and points at the provenance file. `README.md` carries the images and the same disclaimer.

## 2026-09-05 — The first playable build comes before the network

Context: #78. `docs/WORKPLAN.md` §4 scheduled M0.9b Sandbox — build a maze offline, F5 into it as a Sleeper — after M0.8 Netcode and M0.9 Round flow. Its own text says it reuses `DreamInstance` (M0.6) and the god view (M0.7), and "from the title" — a title M0 did not have; the numbering was the only thing putting it after the network. The owner asked whether a playable demonstrator was planned and when, and the honest answer was "yes, and two milestones later than it needs to be".
Decision: M0.9b follows M0.7. A new item, M0.6b (#79), sits between M0.6 and M0.7 and settles the scene architecture — a persistent Boot scene with the `Services` locator, additive loading, a `GameFlow` state machine that owns every transition, input maps per state, a UXML placeholder Title. It cannot come before M0.6, because its acceptance loads the Dream and `DreamInstance` has to exist first; M0.6's own scene is a dev scene built by an editor script, like the gauntlet, and stays one. It comes before M0.7 so the god view is built inside the flow rather than moved into it afterwards. §1 rule 2 now picks work by the plan's order rather than the issue number, because a reorder cannot survive a rule that sorts by tracker number.
Reason: the plan's own goal for M0 is "if this is not at least a little fun, stop and rethink before M1", and the sandbox is the earliest point at which anyone can find out. Netcode is easier to debug against a working offline reference than alongside it. The flow skeleton is pulled forward for the opposite reason: M1's UI work (#26) would otherwise begin by tearing out whatever scene loading M0.7 and M0.9b improvised. The title, lobby, options and pause polish stay in M1 as `docs/UI.md` §16 maps them; only the load-bearing structure moves.
Spec: none. `docs/UI.md` §2 and §12 already describe this flow and this sandbox. `docs/UI.md` §16's M0 line gains the placeholder Title, since the mapping otherwise put every title in M1 while §4 had one in M0. `docs/WORKPLAN.md` §1 rule 2, §4 M0.6b and §4 M0.9b are the other edits.
...
Reason: NC and ND fail `docs/SPEC.md` §18's own test — NC would make the artefact non-commercial, which an MIT repository is not, and ND forbids the rescaling, re-pivoting and material upgrades §17's pipeline performs on everything it ingests. **SA passes the first half of that test and fails the second**: ShareAlike material is redistributable in source form, so the public-repository half of the rationale does not reach it — but the same pipeline that makes ND impossible makes SA propagate. §18 licenses this project's generated shell textures as CC-BY-4.0, and a texture derived from an SA source is Adapted Material that would have to ship SA instead. The first draft of this entry carved out props, on the ground that a prop merely placed in a cube is a Collection rather than an Adaptation; that carve-out does not survive §17, which puts `AssetNormalizer` over everything dropped into `assets/` — rescale to metres, re-pivot to base centre, URP material upgrade — so the prop in the cube is adapted like the texture is. (§17 is the plan; the script is not written yet, and the rule is set by what the pipeline is specified to do rather than by what has been built.) The policy is the load-bearing half either way: the family is refused whole rather than adjudicated asset by asset, so the answer does not move if CC 4.0 §2(a)(4)'s carve-out for technical and format modifications turns out to cover an import-settings rescale. Recorded because a contributor holding a CC BY-SA 4.0 prop is correct that it is redistributable, and until now had no document saying why it was refused anyway.
Spec: `docs/SPEC.md` §18's asset-rule bullet names the three refused clauses and the reason for each, and states the ledger row shape — `| file | source | licence |` — because the gate now reads that row by position and §17 fixes no shape to point at. `CONTRIBUTING.md` and `.github/ISSUE_TEMPLATE/cube.md` say the same in shorter form.
