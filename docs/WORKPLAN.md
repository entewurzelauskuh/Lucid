...
**M0.6b Scene flow skeleton (#79)**
- One persistent Boot scene holding the `Services` locator; every other scene additive, never a single-mode `LoadScene`. A `GameFlow` state machine owns every transition, scenes never load each other; M0 states `Title → Sandbox → Title`, shaped so M0.9's `Lobby → Round → Results → Lobby` are more states rather than a rewrite. Input maps enabled per state. Title is a UXML placeholder with the two entries that work offline, Sandbox and Quit (UI doc §3, §15); the rest of the title, lobby, options and pause is M1 (UI doc §16).
- Accept: from Title, Sandbox loads the Dream additively and returns with nothing leaked — the same `Services` instance on both sides of the round trip, no Dream objects left behind, proven by a PlayMode test; `GameFlow` refuses a transition not in its table; the gauntlet and fog-door dev scenes still load directly in play mode.
